...
Cloud_Farm_project/
├── apps/
│   ├── miniapp/              # Taro 4 + Vue 3              微信小程序 + H5(主战场,25 页全接真后端)
...
> ⚠️ **Windows 上跑 `db:seed` 必须用 PowerShell,不要用 Git Bash**。实测在 Git Bash 里跑 `npm run db:seed`,tsx 执行时会**随机丢弃 UTF-8 字节**,导致中文字段部分乱码(同一批数据里有的字段好、有的坏,如 goods.name 正常但 spec/intro 里个别字丢首字节)。换 PowerShell 跑同一条命令写入完全干净。这解释了历史上两次"seed 乱码重跑修复"——真正起效的是换执行环境。验证中文是否干净别只看 curl 终端输出(可能有显示假象),用 `node -e` + `JSON.stringify` 打印最可靠。
...
- **★ M-08 农产品商城(2026-07-07)** ——
  - **补 PRD 最大 P0 缺口**:认养之外单独买农产品的商城,浏览 → 选 SKU → 购物车 → 结算下单完整闭环
  - **后端**:
    - schema 加 `Goods` + `Sku` 两表(一商品挂多规格,库存/价在 Sku)+ migration `add_goods_sku`
    - seed 7 商品 / 12 SKU(鲜食 fresh / 深加工 processed / 套餐箱 box / 周边 around 四类目)
    - `GoodsModule`:`GET /api/goods[?category]`(列表带 minPrice)+ `/:id`(详情带 SKU),公开只读
    - `OrderService.createShop` + `POST /api/orders/shop`:事务校验地址 + 逐 SKU 校验并扣库存 + 累计销量 + **冷链运费**(冷链品不满 199 加 ¥20,满 199 包邮)+ 可选券(scope=all/shop)→ 生成 `type=产地直送` 订单,复用既有 pay(产地直送 paid→shipped)/ 订单列表 / 详情页
  - **类型 / SDK**:shared 加 `Goods`/`Sku` 类型;api-client 加 `goods.ts`(listGoods/getGoods)+ `orders.createShopOrder`
  - **miniapp(4 新页 + 2 store,页数 21 → 25)**:
    - `stores/goods.js`(list+detail 缓存)、`stores/cart.js`(**纯客户端 localStorage 持久化**,最多 20 SKU,不上后端,结算时才发下单)
    - `pages/shop`:类目 tab + 商品卡 cf-stagger + 悬浮购物车徽标
    - `pages/goods-detail`:SKU 选规格 + 数量步进 + 加购/立即买 + 加购成功动画
    - `pages/cart`:勾选 + 数量 + 删除 + 合计 + 去结算
    - `pages/shop-checkout`:地址 + 清单 + 券 + 冷链运费 + 金额 count-up + 下单跳订单详情
    - home「农产品」入口从占位(跳 profile)改跳 `/pages/shop`;app.config 注册 4 页
  - **回归网**:`test/shop-order.e2e-spec.ts` 4 用例(401 / 正常下单+扣库存 / 冷链运费 / 库存不足 409),**全套 e2e 17 项全过**
  - **验收**:浏览器全链路走查(¥77=39+19×2 / 冷链 +¥20 / 券 -¥20 / 应付 ¥77)+ console 零报错;miniapp build 171 modules
  - ⚠️ **重要环境坑**:本次撞到中文乱码,定位根因 —— 在这台 Windows 上用 **Bash/Git Bash 跑 `db:seed` 会随机损坏 UTF-8 中文,必须用 PowerShell 跑**(详见 §「数据库 seed」注意)
  - 🚧 商城延伸项(售后 / 满减满赠 / Admin 商城订单管理)可后续增量补
...
| **★ M-08 农产品商城** | Goods/Sku 后端 + 商品列表/详情/购物车/结算 4 页 + 冷链运费 + 商城下单 e2e(17 项) | ✅ 已完成 |
| **P5 真萤石云接入** | CameraService 里把 mock 地址换成 EZOPEN OpenAPI 调用 | 待萤石账号 |
| **P6 C 端 Web Portal 拆分** | apps/web/ 独立 Vue 3,翻译现有 25 个页面 | 用户决定暂缓,排在 P5 后 |
